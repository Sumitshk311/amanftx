...
const ProjectDetails = () => {
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen pt-32 pb-20 px-4 md:px-6 text-white bg-[#050505]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-7 flex justify-center">
            {/* ✅ FIXED: Aspect-ratio hata di taaki video apne size mein khule */}
            <div
              onClick={togglePlay}
              className="relative w-full max-w-[800px] rounded-[2rem] overflow-hidden bg-black border border-white/10 shadow-[0_0_80px_rgba(147,51,234,0.2)] cursor-pointer group"
            >
              <video
                ref={videoRef}
                ...
                loop
                playsInline
                className="w-full h-full block object-contain" // ✅ object-contain se video kategi nahi
              />

              <AnimatePresence>
                {showIcon && (
                  <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.2 }} className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
              ...
              <div className="absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-black via-transparent to-transparent flex justify-between items-end">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-purple-500 font-black">FTX Media</p>
          ...
          <div className="lg:col-span-5 space-y-10">
            ...
            <div className="bg-white/[0.03] border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-3xl">
              <div className="flex items-center gap-4 mb-6">
                ...
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">Tools Used</h4>
              </div>
              <div className="flex flex-wrap gap-3">
                {project.tools?.map((tool, i) => (
                  <span key={i} className="px-6 py-3 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-purple-600 transition-all">
                    {tool}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-white/[0.03] border border-white/5 p-8 rounded-[2.5rem] flex items-center gap-4">
              ...
              <div>
                <p className="text-[8px] uppercase tracking-widest text-gray-500 font-black">Project Type</p>
                <p className="text-sm font-black uppercase italic">Cinematic Motion Edit</p>
              </div>
            </div>
            <p className="text-gray-700 text-[9px] uppercase tracking-[0.5em] font-bold">© FTX Media 2026. All Rights Reserved.</p>
          </div>
        </div>
      </div>
    </motion.section>
